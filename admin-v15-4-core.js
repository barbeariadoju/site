...
// admin-notificacoes.html, admin-mensagens.html).
  ...
  // v29.152.0 — o número gravado com o 55 (13 dígitos, vindo do WhatsApp) saía cru na tela
  // ("5511974845870" ao lado de "(11) 97484-5870" — caso Helder). Tira o 55 e formata
  // também o fixo/8 dígitos locais. Só apresentação: o valor gravado não muda.
  ...
  // v29.152.0 — caso Helder (08/09/2026): a lista de clientes era agrupada pelos DÍGITOS
  // EXATOS do telefone. O cadastro dele tem 11974845870; os dois últimos agendamentos, feitos
  // pela JuIA, gravaram 5511974845870. Resultado: DOIS "Helder" na busca do Novo agendamento e
  // no CRM, um deles sem ficha (id nulo) — não era cadastro duplicado no banco (lá o índice
  // único por phone_key já impedia), era o painel tratando o mesmo número como dois. Agora
  // agrupa por phoneKey (DDD + 8 últimos, a mesma regra de phone_match_key do banco). O campo
  // `phone` de cada cliente continua sendo os dígitos gravados (da ficha, quando existe), e é
  // único por cliente — é por ele que a busca e a mesclagem identificam quem foi clicado.
