document.querySelectorAll('[data-copy]').forEach(btn=>{btn.addEventListener('click',async()=>{try{await navigator.clipboard.writeText(btn.dataset.copy);const old=btn.textContent;btn.textContent='Copiado!';setTimeout(()=>btn.textContent=old,1600)}catch(e){alert('Copie manualmente: '+btn.dataset.copy)}})});/* v29.205.0 — revelação a partir de um estado VISÍVEL: o CSS não esconde nada; aqui só o que
   ainda está abaixo da dobra ganha .reveal-pending e aparece ao chegar. Antes, todo .reveal
   nascia invisível e dependia deste script — o texto do hero esperava o JS (LCP atrasado) e,
   se o script falhasse, a página ficava em branco. */
const vistos=new WeakSet();const obs=('IntersectionObserver' in window)?new IntersectionObserver((entries)=>{entries.forEach(e=>{const el=e.target;if(!vistos.has(el)){vistos.add(el);/* 1a leitura (assíncrona, sem forçar layout): só esconde o que está ABAIXO da tela */if(!e.isIntersecting&&e.boundingClientRect.top>0){el.classList.add('reveal-pending');return}obs.unobserve(el);return}if(e.isIntersecting){el.classList.remove('reveal-pending');el.classList.add('visible');obs.unobserve(el)}})},{threshold:.12}):null;if(obs)document.querySelectorAll('.reveal').forEach(el=>obs.observe(el));
/* v29.205.0 — capa do vídeo só quando ele chega perto da tela: os 90 KB eram baixados no
   carregamento inicial disputando banda com o logo, que é o LCP da home. */
(()=>{const vs=document.querySelectorAll('video[data-poster]');if(!vs.length)return;const por=v=>{v.poster=v.dataset.poster;v.removeAttribute('data-poster')};if(!('IntersectionObserver' in window)){vs.forEach(por);return}const io=new IntersectionObserver(es=>es.forEach(e=>{if(e.isIntersecting){por(e.target);io.unobserve(e.target)}}),{rootMargin:'600px 0px'});vs.forEach(v=>io.observe(v))})();;if('serviceWorker' in navigator){window.addEventListener('load',()=>navigator.serviceWorker.register('/sw.js?v=28.16.1',{updateViaCache:'none'}).then(r=>r.update()).catch(()=>{}));let bdjSwReloaded=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(bdjSwReloaded)return;bdjSwReloaded=true;window.location.reload()})}
