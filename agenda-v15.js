...
(() => {
  ...
  function validarDados(){const nome=$('agenda-name').value.trim(),dig=$('agenda-phone').value.replace(/\D/g,''),mail=$('agenda-email').value.trim();const erros=[['agenda-name',!nome?'Digite seu nome.':nome.length<2?'Digite o nome completo — como a gente vai te chamar na cadeira.':''],['agenda-phone',!dig?'Digite seu WhatsApp com DDD.':dig.length<10?'Número incompleto: são 11 dígitos com o DDD, ex.: (11) 91234-5678.':''],['agenda-email',mail&&!$('agenda-email').checkValidity()?'E-mail incompleto — confira o @ e o domínio, ou deixe em branco.':'']];erros.forEach(([id,t])=>avisoCampo(id,t));const primeiro=erros.find(e=>e[1]);if(primeiro){$(primeiro[0]).focus();return false}return true}
  ...
  // v29.205.7 — dia sem horário que caiba aparece como "lotado" na faixa (antes parecia igual aos
  // outros e o cliente só descobria depois de tocar). Consulta leve em segundo plano, com cache.
  const lotacao=new Map();
  function marcarLotados(){const dur=total().duration;document.querySelectorAll("#agenda-day-strip [data-day]").forEach(async ch=>{const k=ch.dataset.day+"|"+dur;if(!lotacao.has(k))lotacao.set(k,fetchAvailableSlots(ch.dataset.day).then(r=>Array.isArray(r)?r.length:(r&&r.slots?r.slots.length:1)).catch(()=>1));const n=await lotacao.get(k);if(n===0){ch.classList.add("is-full");ch.setAttribute("aria-label",ch.getAttribute("aria-label")+" — lotado");if(!ch.querySelector("em"))ch.insertAdjacentHTML("beforeend","<em>lotado</em>")}})}
  function renderDayStrip(){const box=$("agenda-day-strip");if(!box)return;const sel=$("agenda-date").value;const dias=[];let d=firstEligibleDate();for(let i=0;dias.length<7&&i<30;i++){if(isOpenDay(d))dias.push(d);d=addDaysISO(d,1)}box.innerHTML=dias.map(x=>{const dt=new Date(x+"T12:00:00");const sem=dt.toLocaleDateString("pt-BR",{weekday:"short"}).replace(".","");return `<button type="button" class="day-chip${x===sel?" is-selected":""}" data-day="${x}" aria-pressed="${x===sel}" aria-label="${dt.toLocaleDateString("pt-BR",{weekday:"long",day:"numeric",month:"long"})}"><small>${sem}</small><strong>${dt.getDate()}</strong></button>`}).join("");marcarLotados()}
  ...
  // v29.237.0 — clique do Google Ads guardado pelo whatsapp-attrib-v29.js ao chegar pelo anúncio
  // ({v,k,t}, 90 dias). Vai junto com o agendamento pra virar conversão offline quando o atendimento for
  // concluído. Modo privado/sem clique: null, e o agendamento segue igual.
  const cliqueDoAnuncio=()=>{try{const o=JSON.parse(localStorage.getItem('bdj_gclid_v1')||'null');if(!o||!o.v||Date.now()-Number(o.t||0)>90*864e5)return null;return {v:String(o.v),k:['gclid','wbraid','gbraid'].includes(o.k)?o.k:'gclid',t:Number(o.t)||null}}catch(e){return null}};
