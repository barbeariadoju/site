...
// escopo (ver header de admin-v15-4-core.js).
  ...
  function renderServiceMode(){
    ...
    if(!box)return;
    box.innerHTML=rows.length?rows.map(x=>`<article class="service-mode-card ${statusClass(x.status)}"><div class="service-mode-time"><strong>${x.start_time.slice(0,5)}</strong><small>${x.end_time?.slice(0,5)||''}</small></div><div class="service-mode-info"><div><h3>${esc(x.customer_name)}</h3><span class="admin-status ${statusClass(x.status)}">${statusLabel(x.status)}</span></div><p>${esc(x.service_name)}</p><small>${formatPhone(x.customer_phone)}</small>${priceSummaryHtml(x)}${paymentSummaryHtml(x)}${productsHtml(x)}</div><div class="service-mode-actions"><a href="${whatsappLink(x)}" target="_blank" rel="noopener">WhatsApp</a>${x.status==='pending'?`<button data-status="confirmed" data-id="${x.id}">Confirmar</button>`:''}${['pending','confirmed'].includes(x.status)?`<button class="is-primary" data-status="completed" data-id="${x.id}">Concluir</button><button data-reschedule="${x.id}">Remarcar</button><button data-status="no_show" data-id="${x.id}">Ausência</button>`:''}<button data-edit="${x.id}">✎ Editar</button><button data-return="${x.id}">Retorno</button></div></article>`).join(''):'<div class="admin-empty">Nenhum atendimento para hoje.</div>';
