...
// (admin-clientes.html). Ver header de admin-v15-4-core.js.
  ...
  async function confirmMerge(){
    ...
    if(!await BDJ_UX.confirm(`Mesclar ${mergeSelectedCustomer.name} em ${mergeKeepCustomer.name}? O histórico, os pontos e o telefone de ${mergeSelectedCustomer.name} passam para ${mergeKeepCustomer.name} (o telefone fica como "outro telefone" do cadastro), e o cadastro duplicado é apagado. Essa ação não pode ser desfeita.`))return;
    ...
    await loadBaseData();closeMergeModal();renderBirthdaySummary();await loadCrmPhoneAliases();
  }
  ...
  // v29.230.0 (pedido do Juliano, 23/09/2026): segundo telefone no mesmo cadastro. Os outros números
  // vivem em customer_phone_aliases (o banco já trata os dois como o mesmo cliente em toda a JuIA,
  // fidelidade e confirmações); aqui só aparecem no card, entram na busca e se adicionam/removem.
  let crmPhoneAliases=null;
  async function loadCrmPhoneAliases(){const {data,error}=await sb.from('customer_phone_aliases').select('alias_key,customer_id,phone,note');crmPhoneAliases=error?[]:(data||[]);renderCRM()}
  const aliasesOf=c=>(crmPhoneAliases||[]).filter(a=>c.id&&a.customer_id===c.id);
  function filteredCustomers(){const raw=$('crm-search')?.value||'',q=normalizeSearch(raw),digits=phoneDigits(raw);return customers.filter(c=>!q||normalizeSearch(c.name).includes(q)||(digits&&(phoneDigits(c.phone).includes(digits)||aliasesOf(c).some(a=>phoneDigits(a.phone).includes(digits))))||normalizeSearch(c.email||'').includes(q))}
  function renderCRM(){if(crmPhoneAliases===null){crmPhoneAliases=[];loadCrmPhoneAliases()}const rows=filteredCustomers();setText('crm-count',rows.length);const box=$('crm-list');box.innerHTML=rows.length?rows.map(customerCard).join(''):'<div class="admin-empty"><strong>Nenhum cliente encontrado.</strong><small>Revise o nome, telefone ou e-mail pesquisado.</small></div>';bindCustomerCardToggle(box);box.querySelectorAll('[data-history]').forEach(b=>b.onclick=()=>toggleCustomerHistory(b.dataset.history));box.querySelectorAll('[data-add-phone]').forEach(b=>b.onclick=()=>addCustomerPhone(b.dataset.addPhone));box.querySelectorAll('[data-sinal]').forEach(b=>b.onclick=()=>checkSinal(b.dataset.sinal,b));box.querySelectorAll('[data-remove-phone]').forEach(b=>b.onclick=()=>removeCustomerPhone(b.dataset.removePhone,b.dataset.phoneLabel));box.querySelectorAll('[data-audit]').forEach(b=>b.onclick=()=>toggleCustomerAudit(b.dataset.audit,b.dataset.auditPhone));box.querySelectorAll('[data-book-customer]').forEach(b=>b.onclick=()=>{const c=customers.find(x=>x.phone===b.dataset.bookCustomer);sessionStorage.setItem('bdj-prefill-customer',JSON.stringify(c));location.href='admin-agendamento.html?modo=retorno'});box.querySelectorAll('[data-edit-customer]').forEach(b=>b.onclick=()=>openCustomerModal(customers.find(x=>x.phone===b.dataset.editCustomer)));box.querySelectorAll('[data-merge-customer]').forEach(b=>b.onclick=()=>openMergeModal(customers.find(x=>x.phone===b.dataset.mergeCustomer)));box.querySelectorAll('[data-archive-customer]').forEach(b=>b.onclick=()=>archiveCustomer(b.dataset.archiveCustomer));box.querySelectorAll('[data-delete-customer]').forEach(b=>b.onclick=()=>deleteCustomerPermanently(b.dataset.deleteCustomer))}
  ...
  function customerCard(c){const lo=loyaltyFor(c.phone);const last=c.lastDate?formatDate(c.lastDate,{day:'2-digit',month:'2-digit',year:'numeric'}):'Sem atendimento';const email=c.email?`<a href="mailto:${encodeURIComponent(c.email)}">${esc(c.email)}</a>`:'<span>Sem e-mail</span>';const age=ageFromBirth(c.birthDate),birth=c.birthDate?`${birthdayLabel(c.birthDate)}${age!==null?` • ${age} anos`:''}`:'Nascimento não informado';return `<article class="crm-card"><button type="button" class="admin-booking-summary" data-toggle-card aria-expanded="false"><div class="crm-avatar">${esc(c.name.charAt(0).toUpperCase())}</div><span class="admin-booking-summary-main"><strong>${esc(c.name)}</strong><small>${formatPhone(c.phone)} • ${last}</small></span><span class="crm-summary-badges">${c.vip?'<span class="crm-vip-badge">★ VIP</span>':''}<span class="crm-score-badge">Ju Score ${c.juScore}</span>${lo.rewards>0?'<span class="admin-visit-badge is-reward">🎁 Prêmio pra usar</span>':''}</span><span class="admin-booking-chevron">⌄</span></button><div class="admin-booking-detail"><div class="admin-booking-detail-inner crm-detail-inner"><p>${email}</p><p class="crm-birth">🎂 ${esc(birth)}</p>${aliasesOf(c).map(a=>`<p class="crm-profile-line crm-alias-line"><b>📱 Outro telefone:</b> ${esc(formatPhone(a.phone))} <button type="button" class="crm-alias-remove" data-remove-phone="${esc(a.alias_key)}" data-phone-label="${esc(formatPhone(a.phone))}" title="Tirar este telefone do cadastro">remover</button></p>`).join('')}${(c.internalTags||[]).length?`<div class="crm-tags">${c.internalTags.map(t=>`<span>${esc(t)}</span>`).join('')}</div>`:''}${(c.preferredServices||[]).length?`<p class="crm-profile-line"><b>Preferidos:</b> ${c.preferredServices.map(esc).join(', ')}</p>`:''}${Object.values(c.stylePreferences||{}).filter(Boolean).length?`<p class="crm-profile-line"><b>Estilo:</b> ${Object.values(c.stylePreferences).filter(Boolean).map(esc).join(', ')}</p>`:''}${c.notes?`<p class="crm-notes">${esc(c.notes)}</p>`:''}${c.lastFeedback?`<p class="crm-private-feedback"><b>💬 Sugestão privada:</b> ${esc(c.lastFeedback)}</p>`:''}<div class="crm-score-row"><span class="crm-level">${c.level}</span>${c.daysInactive!==null?`<span>${c.daysInactive} dias desde a última visita</span>`:''}</div><div class="crm-stats"><span><strong>${c.completed}</strong> atendimentos</span><span><strong>${money(c.total)}</strong> gasto</span><span><strong>${money(c.avgTicket)}</strong> ticket médio</span><span><strong>${c.reviewClicks||0}</strong> cliques em avaliação</span><span><strong>${c.feedbacks||0}</strong> sugestões</span><span><strong>${c.noShows}</strong> ausências</span><span><strong>${lo.found?`${lo.points}/10`:'–'}</strong> pontos fidelidade${lo.rewards>0?` · 🎁 ${lo.rewards} prêmio${lo.rewards>1?'s':''} pra usar`:''}</span><span><strong>${last}</strong> última visita</span></div><div id="crm-history-${c.phone}" class="crm-history" hidden>${c.history.length?c.history.slice(0,30).map(x=>`<div><span>${formatDate(x.booking_date,{day:'2-digit',month:'2-digit',year:'numeric'})} • ${x.start_time.slice(0,5)}</span><strong>${esc(x.service_name)}${parseProducts(x).length?` + ${parseProducts(x).map(p=>esc(p.name)).join(', ')}`:''}</strong><small>${statusLabel(x.status)} • ${money(Number(x.service_price||0)+Number(x.products_price||0))}</small></div>`).join(''):'<div class="admin-empty">Cliente cadastrado manualmente, sem histórico.</div>'}</div><div id="crm-audit-${c.phone}" class="crm-audit" hidden></div><div class="crm-actions"><button data-history="${c.phone}">Histórico</button><button data-audit="${c.id||''}" data-audit-phone="${c.phone}" ${c.id?'':'disabled'}>🕘 Auditoria</button><button data-edit-customer="${c.phone}">Editar</button><button class="is-primary" data-book-customer="${c.phone}">Agendar</button><a href="${whatsappBusinessUrl(c.phone)}" target="_blank" rel="noopener">WhatsApp</a>${c.email?`<a href="mailto:${encodeURIComponent(c.email)}">E-mail</a>`:''}<button data-add-phone="${c.id||''}" ${c.id?'':'disabled'}>＋ Telefone</button><button data-sinal="${c.phone}" title="Regra do sinal de 50% (dois cancelamentos em cima da hora)">💰 Sinal</button><button data-merge-customer="${c.phone}" ${c.id?'':'disabled'}>🔗 Mesclar</button><button data-archive-customer="${c.id||''}" ${c.id?'':'disabled'}>Arquivar</button><button class="is-danger" data-delete-customer="${c.id||''}" ${c.id?'':'disabled'}>Excluir definitivamente</button></div></div></div></article>`}
  ...
  async function archiveCustomer(id){if(!id)return;if(!await BDJ_UX.confirm('Arquivar este cliente? Ele sairá da lista, mas o histórico será preservado.'))return;const {error}=await sb.rpc('admin_archive_customer',{p_customer_id:id});if(error){alert(error.message);return}await loadBaseData();renderBirthdaySummary();renderCRM()}
  async function addCustomerPhone(id){
    const c=customers.find(x=>x.id===id);if(!c)return;
    const tel=await BDJ_UX.prompt(`Outro telefone de ${c.name} (com DDD). Os agendamentos feitos por ele passam a entrar neste cadastro.`,'');
    if(!tel||!tel.trim())return;
    const {data,error}=await sb.rpc('admin_add_customer_phone',{p_customer_id:id,p_phone:tel.trim()});
    if(error||data?.ok===false){alert(data?.error||error?.message||'Não foi possível adicionar o telefone.');return}
    await loadBaseData();await loadCrmPhoneAliases();
  }
  // v29.240.0 — regra do sinal de 50% (Juliano, 26/09/2026): os dois últimos acontecimentos do cliente foram
  // falta ou cancelamento dele com menos de 24 h. Aqui ele confere e, se quiser, dispensa (o que aconteceu
  // antes da dispensa deixa de contar).
  async function checkSinal(phone,btn){
    if(!phone)return;const c=customers.find(x=>x.phone===phone);
    const antes=btn.textContent;btn.disabled=true;btn.textContent='Conferindo…';
    const {data:ativo,error}=await sb.rpc('admin_sinal_cancelamentos',{p_phone:phone});
    btn.disabled=false;btn.textContent=antes;
    if(error){alert(error.message);return}
    if(!ativo){BDJ_UX.toast(`${c?.name||'Este cliente'} não está na regra do sinal: agenda normalmente.`,'success');return}
    if(!await BDJ_UX.confirm(`${c?.name||'Este cliente'} está na regra do sinal: os dois últimos horários foram falta ou cancelamento em cima da hora. O próximo agendamento pelo WhatsApp ou pelo site pede sinal de 50% pelo Pix.

Dispensar a regra para este cliente? Os cancelamentos até hoje deixam de contar.`))return;
    const {error:e2}=await sb.rpc('admin_dispensar_sinal',{p_phone:phone});
    if(e2){alert(e2.message);return}
    BDJ_UX.toast('Pronto: o próximo agendamento dele sai sem sinal.','success');
  }
  async function removeCustomerPhone(aliasKey,label){
    if(!aliasKey||!await BDJ_UX.confirm(`Tirar ${label||'este telefone'} do cadastro? Os agendamentos que já existem continuam no cadastro; os próximos feitos por esse número voltam a contar como outra pessoa.`))return;
    const {data,error}=await sb.rpc('admin_remove_customer_phone',{p_alias_key:aliasKey});
    if(error||data?.ok===false){alert(data?.error||error?.message||'Não foi possível remover o telefone.');return}
    await loadCrmPhoneAliases();
  }
  async function deleteCustomerPermanently(id){if(!id)return;const c=customers.find(x=>x.id===id);const typed=await BDJ_UX.prompt(`Exclusão definitiva de ${c?.name||'cliente'}:
