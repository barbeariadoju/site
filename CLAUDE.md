...
**Tabela nova no Supabase precisa de GRANT explícito (a partir de 30/10/2026).** Aviso da Supabase de 23/09/2026: tabela criada em `public` depois de 30/10 não recebe mais acesso automático da Data API (supabase-js/PostgREST) — sem GRANT, a API responde "permission denied" e o painel ou a function quebram calados. As tabelas que já existem não mudam. Toda migração que cria tabela passa a incluir, no mesmo arquivo, `grant ... on public.<tabela> to service_role` (sempre, é o que as functions usam) e, só se o site/painel ler direto, `to authenticated`/`to anon` — junto com a RLS de sempre.
