...
// v29.62.0 — regra das famílias (caso Augusto Monteiro, 22/08/2026: "Corte + Barboterapia
...
describe('familiesOf', () => {
  it('classifica cortes, barbas, combos e o pezinho', () => {
    ...
    expect([...familiesOf('Corte + Barboterapia')].sort()).toEqual(['barba', 'corte']);
...
describe('applyServiceRule', () => {
  ...
  it('combo que já inclui a barba recusa Barba Express (caso Augusto)', () => {
    const r = applyServiceRule(['Corte + Barboterapia'], 'Barba Express');
    expect(r.added).toBe(false);
    expect(r.services).toEqual(['Corte + Barboterapia']);
    expect(r.message).toMatch(/já inclui a barba/);
  });
  it('barba em cima de barba: a mais nova substitui', () => {
    expect(applyServiceRule(['Barba Express'], 'Barboterapia').services).toEqual(['Barboterapia']);
    expect(applyServiceRule(['Barboterapia'], 'Barba Express').services).toEqual(['Barba Express']);
  });
  it('combo substitui corte e barba soltos de uma vez', () => {
    const r = applyServiceRule(['Corte de cabelo', 'Barba Express', 'Sobrancelha Masculina'], 'Corte + Barboterapia');
    expect(r.services).toEqual(['Sobrancelha Masculina', 'Corte + Barboterapia']);
...
describe('normalizeServiceSet', () => {
  it('fica com o combo quando corte, combo e barba vêm juntos', () => {
    const r = normalizeServiceSet(['Corte de cabelo', 'Corte + Barboterapia', 'Barba Express']);
    expect(r.items).toEqual(['Corte + Barboterapia']);
    expect(r.removed.map(x => x.name).sort()).toEqual(['Barba Express', 'Corte de cabelo']);
    expect(r.removed.every(x => x.keptBy === 'Corte + Barboterapia')).toBe(true);
  });
  it('entre duas barbas fica a mais completa (maior preço)', () => {
    const r = normalizeServiceSet([{ name: 'Barba Express', price: 25 }, { name: 'Barboterapia', price: 40 }]);
    expect(r.items.map(x => x.name)).toEqual(['Barboterapia']);
  });
...
describe('splitServiceNames', () => {
  const known = ['Corte de cabelo', 'Corte + Barboterapia', 'Barba Express', 'Corte + Lavagem', 'Sobrancelha Masculina'];
  it('não confunde o + do combo com o separador', () => {
    expect(splitServiceNames('Corte + Barboterapia + Barba Express', known)).toEqual(['Corte + Barboterapia', 'Barba Express']);
