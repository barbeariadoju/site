...
import { tipoDoDia, bancoDe, escolherPauta, ganchosGastos, ganchosUsados, GANCHOS_DE_MARCA, REGRAS_EDUCATIVO, REGRAS_HUMOR } from '../_shared/pautas.ts'
...
function themeTextFor(context: Record<string, unknown>, campanha = ''): string {
  switch (context.tipo) {
    ...
    case 'manha_meio_semana':
      ...
      return 'manhã de meio de semana na barbearia: luz de manhã entrando pela vitrine, cadeira pronta e ferramentas alinhadas, ambiente acolhedor e sossegado, sem pessoas e sem texto na imagem.'
    ...
    case 'humor':
      return `cena bem-humorada de barbearia sugerida só por objetos, ligada a esta situação: ${String(context.assunto || '').slice(0, 160)} — sem pessoas, sem rosto, sem texto na imagem.`
    case 'fidelidade':
      return 'clima de recompensa e cuidado contínuo — detalhes do ambiente e do ritual de barbearia, sem texto na imagem.'
    default:
      return 'a experiência de ser bem atendido — acolhimento, poltrona, ferramentas cuidadas, ambiente arrumado, sem texto na imagem.'
  }
}
...
async function generateAndUploadImage(admin: ReturnType<typeof createClient>, geminiKey: string | undefined, themeText: string, postId: string, diaISO: string): Promise<string | null> {
  ...
  try {
    ...
    const DOMINGO_STYLE = `Fotografia still life editorial de altíssimo padrão técnico para o Instagram de uma barbearia masculina bem cuidada em Bragança Paulista/SP. Estética silenciosa e contemplativa: simples, arejada, sem excesso.
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    const seteDiasAtrasISO = new Date(Date.now() - 7 * 24 * 3600 * 1000).toISOString()
    const { data: semanaIa } = await admin
      .from('content_posts')
      ...
      .lt('created_at', startOfTodayISO)
    const gastos = ganchosGastos((semanaIa || []).map((r) => ({
      dia: new Date(String(r.created_at)).toLocaleDateString('en-CA', { timeZone: 'America/Sao_Paulo' }),
      texto: String(r.caption || ''),
    })), 1)
    const nomesGastos = GANCHOS_DE_MARCA.filter((g) => gastos.includes(g.id)).map((g) => g.nome)
    const avisoGanchos = nomesGastos.length
      ? `GANCHOS JÁ USADOS NESTA SEMANA — PROIBIDO hoje, nem de passagem: ${nomesGastos.join('; ')}. Escolha outro fato concreto.`
      : ''
    ...
    const temaRepetido = (candidato: string): string[] => {
      const gasto = ganchosUsados(candidato, gastos)
      if (gasto.length) return gasto
...
A CONVERSA DA CADEIRA NÃO É ASSUNTO PÚBLICO (ordem do Juliano em 10/09/2026, depois de corrigir uma legenda à mão): é PROIBIDO prometer ou afirmar publicamente sigilo, confidência ou intimidade sobre o que o cliente fala na cadeira — "a conversa que nós tivemos fica entre eu e você", "o que se fala aqui fica aqui", "segredo de barbeiro", "aqui você pode desabafar", "a cadeira é seu confessionário", "terapia de barbearia". Isso insinua que existe conversa íntima a proteger e trata o cliente como alguém que se expõe. O post fala do corte, da barba, do cuidado e da casa; o que o cliente conversa na cadeira nunca vira tema, promessa ou piada.`
    ...
    if (tipoDia === 'educativo' || tipoDia === 'humor') {
      const { data: usadasRows } = await admin
        .from('content_posts')
        .select('context')
        .eq('source', 'ia')
        .eq('platform', 'instagram')
        .gte('created_at', new Date(Date.now() - 200 * 24 * 3600 * 1000).toISOString())
        .order('created_at', { ascending: false })
        .limit(400)
      const usadas = (usadasRows || []).map((r) => String((r.context as Record<string, unknown> | null)?.pauta || '')).filter(Boolean)
      pautaHoje = escolherPauta(bancoDe(tipoDia), usadas)
    }
    const tipoHoje = pautaHoje ? tipoDia : null
...
${tipoHoje === 'educativo' ? REGRAS_EDUCATIVO : REGRAS_HUMOR} Isto vale ACIMA de qualquer instrução de "vender a experiência" ou "motivo pra agendar": hoje o post entrega ${tipoHoje === 'educativo' ? 'conhecimento útil' : 'uma risada'}, e a marca ganha por isso. ${NO_AGENDA_TALK}`
      context = { tipo: tipoHoje, pauta: pautaHoje!.id, assunto: pautaHoje!.tema.slice(0, 160), dia: todaySP }
      ...
      if (rotation === 0) {
        ...
        // diretor de escola: é o oceano azul da casa). v29.78.1 — o Juliano LIBEROU a palavra
        // "exclusivo" (diferente de "premium", que segue proibido: exclusivo é fato, premium
        // é auto-elogio) — mas SEMPRE ancorada no fato que a prova, nunca solta como enfeite.
        contextFact = `Tema de hoje: a EXPERIÊNCIA real de ser atendido na Barbearia do Ju — escolha 1 (no máximo 2) destes fatos verdadeiros e construa o texto NELE, sem listar os outros: o ATENDIMENTO EXCLUSIVO (um barbeiro, um cliente por vez — a barbearia inteira é sua na sua hora, você chega e o Juliano está pronto te esperando, e a conversa na cadeira não tem plateia); café na chegada; atendimento com hora marcada respeitada (sem fila e sem espera); atendimento sem pressa; ambiente climatizado; cartão fidelidade (a cada 10 cortes, 1 é grátis). REGRA da palavra "exclusivo": pode usar, mas SEMPRE colada no fato que a prova ("atendimento exclusivo: um cliente por vez") — nunca solta como enfeite ("experiência exclusiva", "ambiente exclusivo" são proibidos). ${VOZ_CONCRETA} ${NO_AGENDA_TALK}`
        ...
        if (featured) {
          ...
          contextFact = `Tema de hoje: a EXPERIÊNCIA real de ser atendido na Barbearia do Ju — escolha 1 (no máximo 2) destes fatos verdadeiros e construa o texto NELE: o atendimento exclusivo (um barbeiro, um cliente por vez, a barbearia inteira é sua na sua hora, o Juliano pronto te esperando, conversa sem plateia), café na chegada, hora marcada respeitada, atendimento sem pressa, ambiente climatizado. "Exclusivo" só colado no fato que a prova ("atendimento exclusivo: um cliente por vez"), nunca solto como enfeite. ${VOZ_CONCRETA} ${NO_AGENDA_TALK}`
          context = { tipo: 'experiencia' }
        }
      }
    }
    ...
    if (!diaEmocionalAviso && (avisoAntiRepeticao || avisoGanchos)) {
      contextFact = `${contextFact}

${[avisoAntiRepeticao, avisoGanchos].filter(Boolean).join('\n')}`
    }
