...
-- Cria bloqueios de dia inteiro ou intervalos de horário e integra com a disponibilidade pública.
...
begin
 ...
 while m<=close_m loop
  ...
  if not exists(select 1 from public.schedule_blocks bl where bl.block_date=p_date and (bl.all_day or (s<bl.end_time and e>bl.start_time)))
     and not exists(select 1 from public.bookings b where b.booking_date=p_date and b.status in ('pending','confirmed') and s<b.end_time and e>b.start_time)
  then slot_time:=s; return next; end if;
  m:=m+15;
 end loop;
end $$;
